...
def play_game(word_list):
    ...
    while True: # keep looping until the player wins or loses
        if game.num_lives == 0: # check if lives counter is 0
            print("You lost")
            return
        elif game.num_letters > 0: # if there are still letters to be guessed
            #print(game.num_lives)
            game.ask_for_input() # ask for another letter
        else:
            print('Congratulations. You won the game!') # means there are 0 letters to guess left in game.num_letters
            return

play_game(["banana", "apple", "plum", "grapefruit", "orange"])
